...
When('I make a POST request to {string} with the order data for update', async function (endpoint) {
  response = await axios.post(`http://localhost:5004/api/order`, OrderMockData, {
    headers: {
...
When('I update the order status to {string}', async function (status) {
  response = await axios.patch(
    `http://localhost:3003/api/order/${createdOrderId}`,
    {
      status: status,
    },
    {
...
Then('delete created order', async function () {
  response = await axios.delete(`http://localhost:3003/api/order/${createdOrderId}`);
